function [sysn,sysm] = gnlcf(sys,options)
%GNLCF   Generalized normalized left coprime factorization.
%    [SYSN,SYSM] = SNLCF(SYS,,OPTIONS) calculates for the transfer 
%    function matrix
%                                       -1
%               G(lambda) =  C(lambdaE-A) B + D
%
%    of a given system SYS = (A-lambdaE,B,C,D) a normalized
%    left coprime factorization
%                                   -1
%               G(lambda) = M(lambda) *N(lambda)
% 
%    where N(lambda) and M(lambda) are stable transfer functions 
%    such that [N(lambda) M(lambda)] is co-inner.
%
...
% compute the coinner coimage of [G(lambda) I] 
[p,m] = size(sys);
sysr = gcrange([sys eye(p)],struct('tol',tol,'coinner',true,'balance',balance));
...
% extract the factors
sysn = sysr(1:p,1:m); sysm = sysr(1:p,m+1:end);
