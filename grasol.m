...
%                     true  - compute a H2-norm optimal solution 
%                     false - compute a Hinf-norm optimal solution (default) 
%   OPTIONS.gamma   - specifies gamma > 0, the desired sub-optimality 
%                     degree to solve the gamma-suboptimal model-matching
%                     problem 
%                        ||G(lambda)*X(lambda)-F(lambda)||_inf <= gamma
%                     (Default: [] in which case, the optimal error  
...
% 
%   [SYSX,INFO] = GRASOL(SYSGF,MF,OPTIONS) uses the compound realization 
...
if ~isa(sysf,'ss')
   if ~isa(sysf,'double') 
      ...
      validateattributes(sysf,{'double'},{'integer','scalar','>=',0,'<=',size(sysg,2)},'','MF')
      mf = sysf;
      m = m-mf;
      sysf = gir(sysg(:,m+1:m+mf),tol);
      sysg = gir(sysg(:,1:m),tol);
   end
else
   if size(sysg,1) ~= size(sysf,1) 
...
% compute the extended outer-co-inner factorization
[Gi,Go,info1] = giofac(sysg,struct('tol',tol,'offset',offset)); 
ro = info1.nrank; 

% detect nonstandard problem 
%nonstandard = order(grange(Go,struct('tol',tol,'zeros','s-unstable'))) > 0;
...
if H2sol
   ...
   if ~discr && norm(F.d(ro+1:end,:)) < told
     % set feedthrough matrix to zero
      F.d(ro+1:end,:) = zeros(p-ro,size(sysf,2)); 
   end
   % solve the H2-LDP min||[ F1-Xt; F2 ] ||_2
   [Xt,Xtu] = gsdec(F(1:ro,:),struct('tol',tol,'job','stable'));
   gopt = norm(grcfid(gir([Xtu; F(ro+1:end,:)],tol)),2); 
else
   % solve the H_inf-LDP min ||[ F1-Xt; F2 ] ||_inf
   opts = struct('tol',tol,'reltol',reltol,'gamma',gamma);
   [Xt,gopt] = glinfldp(F.',p-ro,opts); Xt = Xt.';
end

if ro == m
   sysx = gir(Go\Xt,tol);
else
   opts_grsol = struct('tol',tol,'sdeg',sdeg,'poles',poles,'mindeg',mindeg);
   sysx = grsol(Go,Xt,opts_grsol); 
